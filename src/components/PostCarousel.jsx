...
export default function PostCarousel({ slides = [], autoplay = false, autoplayDelay = 4000, showControls = true, openLightbox, disableLightbox = false }) {
  ...
  // Local fallback lightbox state when no openLightbox prop is provided.
  const [localLightbox, setLocalLightbox] = useState(null);
                ...
                onClick={(e) => {
                  ...
                  if (isPrev) { if (emblaApiRef.current) emblaApiRef.current.scrollPrev(); }
                    ...
                    if (canOpenLightbox) openLightbox({ src: s.src, alt: s.alt || "", caption: s.caption || null });
                      ...
                      // fallback: open a local lightbox modal (body class handled by LocalLightbox)
                      setLocalLightbox({ src: s.src, alt: s.alt || "", caption: s.caption || null });
                    }
                  }
                }}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    ...
                    if (isPrev) { if (emblaApiRef.current) emblaApiRef.current.scrollPrev(); }
                      ...
                      if (canOpenLightbox) openLightbox({ src: s.src, alt: s.alt || "", caption: s.caption || null });
                      else { setLocalLightbox({ src: s.src, alt: s.alt || "", caption: s.caption || null }); }
                    }
                  }
                }}
              />
...
// render a small local lightbox portal (client-only) via adding to document.body
function LocalLightbox({ data, onClose }) {
  useEffect(() => {
    function onKey(e) {
      if (e.key === 'Escape') onClose();
    }
    window.addEventListener('keydown', onKey);
    // ensure body class while lightbox is open
    try { document.body.classList.add('lightbox-open'); } catch (e) {}
    return () => {
      window.removeEventListener('keydown', onKey);
      try { document.body.classList.remove('lightbox-open'); } catch (e) {}
    };
  }, [onClose]);
  ...
  const content = (
    <div className="post-lightbox-overlay" onClick={onClose} role="dialog" aria-modal="true">
      <img src={data.src} alt={data.alt || ''} style={{ maxWidth: '90vw', maxHeight: '90vh', objectFit: 'contain' }} />
      {data.caption && <div style={{ position: 'absolute', bottom: 20, color: '#fff' }}>{data.caption}</div>}
    </div>
  );

  // render at document.body so fixed positioning is relative to viewport
